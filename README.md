...
| move_file | 파일/폴더 이동, 이름 변경, 휴지통 복구 |
| trash_file | 삭제 대신 서버의 /.mcp-trash로 이동 |
...
삭제한 파일은 반환된 `trash_path`에서 원래 경로로 `move_file`을 호출해 복구합니다.
자동 파일 백업은 `list_file_backups`에서 찾고 `restore_file_backup`으로 복원합니다. 아래 복원 절차를 참고하세요.
백업 및 휴지통은 자동 정리하지 않으므로 디스크 공간을 차지합니다.

강제 종료(kill), 영구 삭제, 사용자 생성, 서버 생성 전용 도구는 포함하지 않습니다.
